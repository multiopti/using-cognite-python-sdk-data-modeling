...
def generate_printer_event(client, cfg: dict, start_ms: int, end_ms: int, last_hour_start_local: datetime, asset_id: int) -> dict:
    ...
    print(f"  --> Successfully posted Printer Event: '{ext_id}' (CDF ID: {cdf_id})")
    return {"code": printer_code, "status": "ok", "external_id": ext_id, "id": cdf_id}


def generate_di_event(client, cfg: dict, start_ms: int, end_ms: int, last_hour_start_local: datetime, asset_id: int) -> dict:
    ctx = _shift_context(last_hour_start_local)
    ...
    ext_id = res.external_id if hasattr(res, "external_id") else res[0].external_id
    cdf_id = res.id if hasattr(res, "id") else res[0].id
    print(f"  --> Successfully posted D&I Event: '{ext_id}' (CDF ID: {cdf_id})")
    return {"code": machine_code, "status": "ok", "external_id": ext_id, "id": cdf_id}


def generate_standum_event(client, cfg: dict, start_ms: int, end_ms: int, last_hour_start_local: datetime, asset_id: int) -> dict:
    ctx = _shift_context(last_hour_start_local)
    machine_code = cfg["code"]
    ...
    ext_id = res.external_id if hasattr(res, "external_id") else res[0].external_id
    cdf_id = res.id if hasattr(res, "id") else res[0].id
    print(f"  --> Successfully posted Standum Event: '{ext_id}' (CDF ID: {cdf_id})")
    return {"code": machine_code, "status": "ok", "external_id": ext_id, "id": cdf_id}


def generate_minster_event(client, cfg: dict, start_ms: int, end_ms: int, last_hour_start_local: datetime, asset_id: int) -> dict:
    """
    ...
    ext_id = res.external_id if hasattr(res, "external_id") else res[0].external_id
    cdf_id = res.id if hasattr(res, "id") else res[0].id
    print(f"  --> Successfully posted MINSTER Event: '{ext_id}' (CDF ID: {cdf_id})")
    return {"code": machine_code, "status": "ok", "external_id": ext_id, "id": cdf_id}


def generate_ispray_event(client, cfg: dict, start_ms: int, end_ms: int, last_hour_start_local: datetime, asset_id: int) -> dict:
    """
    ...
    ext_id = res.external_id if hasattr(res, "external_id") else res[0].external_id
    cdf_id = res.id if hasattr(res, "id") else res[0].id
    print(f"  --> Successfully posted ISPRAY Event: '{ext_id}' (CDF ID: {cdf_id})")
    return {"code": machine_code, "status": "ok", "external_id": ext_id, "id": cdf_id}
...
def run_all_production_reports(client, data: dict = None) -> dict:
    ...
    for cfg in configs:
        ...
        try:
            results.append(generator(client, cfg, start_ms, end_ms, last_hour_start_local, asset_id))
...
def handle(client: "CogniteClient" = None, data: dict = None) -> dict:  # noqa: F821 - injected by CDF at runtime
    ...
    `data` accepts (hours_ago, machine_codes).
    """
