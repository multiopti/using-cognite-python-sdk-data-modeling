"""
Local-dev-friendly Cognite client factory for the overview_dashboard/ app.
...
When you run the same code locally (`streamlit run overview_dashboard/main.py`
in a normal terminal), there is no Fusion runtime behind it, so the
zero-argument call has nothing to authenticate with. This module tries the
zero-arg form first (so the exact same code works once deployed) and falls
back to an explicit OAuth client-credentials config built from a local .env
file otherwise.
...
3. `pip install python-dotenv` (already in overview_dashboard/requirements.txt).
